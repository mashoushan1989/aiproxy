...
- `channel_id`, if provided, must exist and include the model in that channel's model list.
...
- bulk updates must skip `price_locked=true` records by default.
- bulk updates may override locked prices only when the request explicitly sends a force flag such as `override_locked=true`.
...
5. Otherwise fall back to existing group model override and base model price behavior.
...
- Bulk import and bulk discount operations show `Skip locked prices` enabled by default.
- Bulk operations can expose `Override locked prices`, but it must be an explicit opt-in and should be visually distinct from the default path.
...
- Imported or auto-calculated discount prices should not become locked unless the import payload or admin action explicitly requests it.
