...
func storeValid(store adaptor.StoreCache, now time.Time) bool {
	return store.ID != "" && (store.ExpiresAt.IsZero() || store.ExpiresAt.After(now))
}
...
func saveStableStoreMapping(
	...
	existing, err := store.GetStore(meta.Group.ID, meta.Token.ID, id)
	if err == nil && storeValid(existing, now) {
		return nil
	}
...
func saveRecentStoreMapping(
	...
	if err == nil && storeValid(existing, now) &&
		...
		now.Sub(existing.UpdatedAt) < minInterval {
		return nil
	}
