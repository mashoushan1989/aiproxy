...
func TestGetRequestURL_ResponsesIgnoresUnrelatedPathBaseMap(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	m := newContractMeta(mode.Responses)
	m.ChannelConfigs = model.ChannelConfigs{
		model.ChannelConfigPathBaseMapKey: map[string]any{
			PathPrefixWebSearch: "https://api.novita.ai/v3",
		},
	}
	...
	const want = "https://api.novita.ai/openai/v1/responses"
	if got.URL != want {
...
func TestGetRequestURL_ResponsesPreservesQuery(t *testing.T) {
	...
	const want = "https://api.novita.ai/openai/v1/responses/resp_123/input_items?limit=20&after=item_1"
	if got.URL != want {
		t.Fatalf("URL: want %s, got %s", want, got.URL)
	}
}
