...
func GetPendingAsyncUsagesDue(limit int, now time.Time) ([]*AsyncUsageInfo, error) {
	var infos []*AsyncUsageInfo

	err := LogDB.
		Where("status = ?", int(AsyncUsageStatusPending)).
		Where(LogDB.Where("next_poll_at <= ?", now).Or("next_poll_at IS NULL")).
...
func RenewAsyncUsageClaim(id int, token string, leaseUntil time.Time) (bool, error) {
	if id == 0 || token == "" {
		return false, nil
	}
...
func AsyncUsageBackoffDelay(retryCount int) time.Duration {
	if retryCount <= 1 {
...
func updateClaimedAsyncUsageInfo(info *AsyncUsageInfo, updates map[string]any) error {
